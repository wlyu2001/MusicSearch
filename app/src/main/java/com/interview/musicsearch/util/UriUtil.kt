...
object UriUtil {
    ...
    private val sURIMatcher = UriMatcher(UriMatcher.NO_MATCH).apply {
        ...
        addURI(null, "search/artists", SEARCH_ARTIST)
        addURI(null, "artist/#/albums", ARTIST_ALBUMS)
        addURI(null, "album/#/tracks", ALBUM_TRACKS)
    }
    ...
    fun getContentUri(type: Int, id: String = ""): Uri {
        return when(type) {
            SEARCH_ARTIST ->
                Uri.Builder().scheme(SCHEME).path("search/artists").build()
            ARTIST_ALBUMS ->
                Uri.Builder().scheme(SCHEME).path("artist/$id/albums").build()
            ALBUM_TRACKS ->
                Uri.Builder().scheme(SCHEME).path("album/$id/tracks").build()
            else ->
